...
def get_image(filename, width, height):
    im = Image.open(filename).resize((width, height), resample=Image.Resampling.LANCZOS)
...
def Resize_Video(frame, w_, h_):
    ...
    pilImage = pilImage.resize((w_, h_), resample=Image.Resampling.LANCZOS)
...
if __name__ == '__main__':
    ...
    # 添加按键
    choose = tk.Button(open_box, width=35, text="选择图像", bg="lightgray", command=path_select)
    ...
    # 打开摄像头
    open_camera = tk.Button(camera_box, text="开启摄像头", width=10, bg="lightgray", command=Open_camera)
    ...
    button_box = tk.Frame(work_box, width=300, height=370, borderwidth=0, bg=bg)
